...
def create_outreach(req: CreateOutreachRequest, user_id: str = Depends(get_user_id)):
    ...
    result = sb.table("outreach_entries").insert(entry).execute()
...
def delete_outreach(entry_id: str, user_id: str = Depends(get_user_id)):
    sb = get_supabase()
    sb.table("outreach_entries").delete().eq("id", entry_id).eq(
        "session_id", user_id
    ).execute()
    return {"ok": True}
